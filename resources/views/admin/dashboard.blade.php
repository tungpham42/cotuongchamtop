...
<div class="space-y-8">
    ...
    <section>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
            ...
            <a href="{{ route('admin.games.index') }}" class="group rounded-2xl border border-slate-200 bg-white p-5 shadow-soft transition hover:-translate-y-1 hover:border-teal-200 hover:shadow-lift">
                <div class="flex items-start justify-between">
                    <span class="h-11 w-11 rounded-2xl bg-teal-50 text-teal-600 flex items-center justify-center">
                        <i class="fa-solid fa-book-open"></i>
                    </span>
                    <i class="fa-solid fa-arrow-up-right-from-square text-slate-300 group-hover:text-teal-500"></i>
                </div>
                <div class="mt-4 text-sm font-extrabold text-slate-900">Games library</div>
                <div class="mt-1 text-xs leading-5 text-slate-500">Review and curate saved & shared games.</div>
            </a>
    ...
    <section>
        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-soft">
                <div class="flex items-center justify-between">
                    <span class="text-xs font-bold uppercase tracking-wide text-slate-400">Saved games</span>
                ...
                <div class="mt-4 flex items-end justify-between gap-3">
                    <div>
                        <div class="text-3xl font-extrabold tracking-tight text-slate-900">{{ number_format($stats['total_games']) }}</div>
                        <div class="mt-2 text-xs text-slate-400">{{ number_format($stats['new_games_month']) }} added this month</div>
                    </div>
                    <div class="text-right shrink-0">
                        <div class="text-[11px] font-extrabold text-slate-400">{{ number_format($stats['total_game_views']) }} views</div>
                    </div>
                </div>
            </div>
    ...
    <section>
        <div class="flex items-center justify-between mb-4">
            <div>
                ...
                <h3 class="mt-1 text-lg font-extrabold text-slate-900">Latest articles & games</h3>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-2 gap-4">
            <div class="rounded-2xl border border-slate-200 bg-white shadow-soft overflow-hidden">
                <div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
                    <div>
                        <h4 class="font-extrabold text-slate-900">Latest articles</h4>
                        <p class="text-xs text-slate-400 mt-1">Newest published and draft posts</p>
                    </div>
                    <a href="{{ route('admin.articles.index') }}" class="text-xs font-extrabold text-indigo-600 hover:text-indigo-800">View all</a>
                </div>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white shadow-soft overflow-hidden">
                <div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
                    <div>
                        <h4 class="font-extrabold text-slate-900">Latest games</h4>
                        <p class="text-xs text-slate-400 mt-1">Newest saved & shared games</p>
                    </div>
                    <a href="{{ route('admin.games.index') }}" class="text-xs font-extrabold text-indigo-600 hover:text-indigo-800">View all</a>
                </div>
                ...
                <div class="divide-y divide-slate-100">
                    @forelse($recentGames as $game)
                        <div class="px-5 py-3.5 flex items-center gap-3">
                            ...
                            <div class="min-w-0 flex-1">
                                <div class="flex items-center gap-2">
                                    <span class="text-sm font-bold text-slate-800 truncate">{{ $game->title }}</span>
                                </div>
                                <div class="text-xs text-slate-400 mt-1 truncate">
                                    {{ $game->user->name ?? 'Anonymous' }} · {{ number_format($game->views) }} views
                                </div>
                            </div>
                            <div class="flex items-center gap-3 shrink-0">
                                <div class="text-[10px] font-semibold text-slate-400">{{ $game->created_at->diffForHumans() }}</div>
                                <a href="{{ route('admin.games.edit', $game->id) }}" class="text-xs font-bold text-indigo-600 hover:text-indigo-800">Edit</a>
                            </div>
                        </div>
                    @empty
                        <div class="px-5 py-10 text-center">
                            <div class="mx-auto h-12 w-12 rounded-2xl bg-teal-50 text-teal-600 flex items-center justify-center">
                                <i class="fa-solid fa-chess-board"></i>
                            </div>
                            <div class="mt-3 text-sm font-bold text-slate-800">No games yet</div>
                            <div class="mt-1 text-xs text-slate-400">Saved or shared games will show up here.</div>
                            <a href="{{ route('admin.games.index') }}" class="mt-3 inline-block text-xs font-extrabold text-indigo-600 hover:text-indigo-800">Go to games library <i class="fa-solid fa-arrow-right ml-1"></i></a>
                        </div>
                    @endforelse
                </div>
            </div>
