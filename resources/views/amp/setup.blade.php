...
<p class="w-100 text-center mt-4">
  <a style="color: white" id="new-board" class="w-25 btn btn-dark btn-lg"><i class="fad fa-save"></i> {{ __("Lưu bàn cờ") }}</a>
  <a style="color: white" id="undo" class="w-25 btn btn-danger btn-lg"><i class="fad fa-undo"></i> {{ __("Xếp lại") }}</a>
</p>
@if ($board != '')
...
$('#new-board').on('click', function(){
  window.location.href = "{{ url('/amp/co-the/') }}/" + board.fen();
});
$('#undo').on('click', undo);
