@php
use App\Models\ChatMessage;

// NOTE: session_start() should typically be handled by Laravel's session middleware,
// but it is included here as it was in the original file.
session_name('CoTuong-'.$roomCode);
session_start();

// Initialize room messages if first time
$messagesCount = ChatMessage::where('room_code', $roomCode)->count();
if ($messagesCount == 0) {
    // Translation: 'Phòng được tạo' -> '채팅방이 생성되었습니다'
    ChatMessage::addMessage($roomCode, 'System', '채팅방이 생성되었습니다', 'system');
}

if (isset($_GET['logout'])) {
    if (isset($_SESSION['name'])) {
        // Translation: 'đã rời khỏi phòng chat' -> '채팅방을 나갔습니다'
        ChatMessage::addMessage($roomCode, $_SESSION['name'], '채팅방을 나갔습니다', 'leave');
        $_SESSION = [];
        setcookie('cotuong_name', '', time() - 3600, "/");
    }
}

if (isset($_POST['enter'])) {
    if ($_POST['name'] != "") {
        $_SESSION['name'] = stripslashes(htmlspecialchars($_POST['name']));
        setcookie('cotuong_name', $_SESSION['name'], time() + (86400 * 30), "/");
        // Translation: 'đã vào phòng chat' -> '채팅방에 입장했습니다'
        ChatMessage::addMessage($roomCode, $_SESSION['name'], '채팅방에 입장했습니다', 'enter');
    }
}

// Get recent messages for display
$chatMessages = ChatMessage::getMessagesForRoom($roomCode);
@endphp

<style>
#loginform form, #chat-wrapper form {
    padding: 9px 0;
    display: block;
    font-size: 14px;
}

#loginform form label, #chat-wrapper form label {
    font-size: 14px;
    font-weight: bold;
    margin-top: 5px;
}

#chat-wrapper {
    margin: 0 0 1rem 0;
    padding-bottom: 0;
    background: #413e3b;
    border: 2px solid #413e3b;
    border-radius: 8px;
    color: #eee;
    height: 350px;
}

#loginform {
    padding-top: 18px;
    text-align: center;
    border: none;
    font-size: 14px;
}

#loginform p {
    padding: 0;
    font-size: 14px;
    font-weight: bold;
}

#chatbox {
    text-align: left;
    margin: 0 auto;
    padding: 10px;
    background: #fff;
    height: calc(100% - 120px);
    width: calc(100% - 20px);
    border: 1px solid #a7a7a7;
    overflow: auto;
    border-radius: 4px;
    border-bottom: 4px solid #a7a7a7;
    font-size: 13px;
}

#loginform + #chatbox {
    margin-bottom: 10px !important;
}

#usermsg {
    border-radius: 4px;
    border: 1px solid #ff9800;
    margin-left: 9px;
    width: calc(100% - 72px);
    font-size: 16px;
    padding: 4px 8px;
}

#name {
    border-radius: 4px;
    border: 1px solid #ff9800;
    padding: 6px 12px;
    font-size: 16px;
    width: calc(100% - 118px);
}

#submitmsg,
#enter {
    background: #ff0028;
    border: 2px solid #ff0028;
    color: white;
    padding: 4px 8px;
    font-weight: bold;
    border-radius: 4px;
    font-size: 12px;
    margin-right: 0;
}

.error {
    color: #ff0000;
    width: 100%;
    text-align: center;
}

#menu {
    padding: 9px;
    display: flex;
    background: #536dfe;
    border-radius: 6px 6px 0 0;
}

#menu p.welcome {
    flex: 1;
    margin: 0;
    font-weight: bold;
}

a#exit {
    color: white;
    background: #c62828;
    padding: 4px 8px;
    border-radius: 4px;
    font-weight: bold;
    text-decoration: none;
}

.msgln {
    margin: 0 0 5px 0;
    color: #413e3b;
}

.msgln span.welcome-info {
    color: goldenrod;
}

.msgln span.left-info {
    color: orangered;
}

.msgln span.enter-info {
    color: green;
}

.msgln span.chat-time {
    color: #666;
    font-size: 60%;
}

.msgln b.user-name, .msgln b.user-name-left, .msgln b.user-name-enter {
    font-weight: bold;
    background: #546e7a;
    color: white;
    padding: 2px 4px;
    font-size: 90%;
    border-radius: 4px;
    margin: 0 5px 0 0;
}

.msgln b.user-name-left {
    background: orangered;
}

.msgln b.user-name-enter {
    background: green;
}
</style>

@php
if (!isset($_SESSION['name'])) {
@endphp
<div id="chat-wrapper">
    <div id="loginform">
        <p>채팅을 위해 이름을 입력하세요!</p> {{-- 'Nhập tên của bạn để chat!' -> '채팅을 위해 이름을 입력하세요!' --}}
        <form id="login-form" method="post" action="{{ url()->current() }}">
            @csrf
            <label for="name">이름 :</label> {{-- 'Tên &#58;' -> '이름 :' --}}
            <input type="text" name="name" id="name" value="{{ isset($_COOKIE['cotuong_name']) ? $_COOKIE['cotuong_name'] : '' }}" />
            <input type="submit" name="enter" id="enter" value="입장" /> {{-- 'Vào' -> '입장' --}}
        </form>
        <div id="login-error" class="error"></div>
    </div>
    <div id="chatbox">
        @foreach($chatMessages as $msg)
            <div class='msgln'>
                <span class='chat-time'>{{ $msg->created_at->format('Y-m-d | H:i:s') }}</span>
                @if($msg->type == 'system')
                    <span class='welcome-info'>{{ $msg->message }}</span>
                @elseif($msg->type == 'enter')
                    <span class='enter-info'>사용자 <b class='user-name-enter'>{{ $msg->username }}</b> {{ $msg->message }}</span> {{-- 'Người dùng' -> '사용자' --}}
                @elseif($msg->type == 'leave')
                    <span class='left-info'>사용자 <b class='user-name-left'>{{ $msg->username }}</b> {{ $msg->message }}</span> {{-- 'Người dùng' -> '사용자' --}}
                @else
                    <b class='user-name'>{{ $msg->username }}</b>: {{ $msg->message }}
                @endif
                <br>
            </div>
        @endforeach
    </div>
</div>
@php
} else {
@endphp
<div id="chat-wrapper">
    <div id="menu">
        <p class="welcome">안녕하세요, <b>@php echo $_SESSION['name']; @endphp</b></p> {{-- 'Chào,' -> '안녕하세요,' --}}
        <p class="logout"><a id="exit" href="javascript:void(0);">채팅 종료</a></p> {{-- 'Thoát chat' -> '채팅 종료' --}}
    </div>
    <div id="chatbox">
        @foreach($chatMessages as $msg)
            <div class='msgln'>
                <span class='chat-time'>{{ $msg->created_at->format('Y-m-d | H:i:s') }}</span>
                @if($msg->type == 'system')
                    <span class='welcome-info'>{{ $msg->message }}</span>
                @elseif($msg->type == 'enter')
                    <span class='enter-info'>사용자 <b class='user-name-enter'>{{ $msg->username }}</b> {{ $msg->message }}</span> {{-- 'Người dùng' -> '사용자' --}}
                @elseif($msg->type == 'leave')
                    <span class='left-info'>사용자 <b class='user-name-left'>{{ $msg->username }}</b> {{ $msg->message }}</span> {{-- 'Người dùng' -> '사용자' --}}
                @else
                    <b class='user-name'>{{ $msg->username }}</b>: {{ $msg->message }}
                @endif
                <br>
            </div>
        @endforeach
    </div>
    <form name="message" id="message-form">
        <input name="usermsg" type="text" id="usermsg" required="required" placeholder="메시지를 입력하세요..." /> {{-- 'Nhập tin nhắn...' -> '메시지를 입력하세요...' --}}
        <input name="submitmsg" type="submit" id="submitmsg" value="전송" /> {{-- 'Gửi' -> '전송' --}}
    </form>
</div>
@php
}
@endphp

<script>
// jQuery Document
$(document).ready(function () {
    // Store current username in JavaScript for client-side use
    let currentUser = "{{ isset($_SESSION['name']) ? addslashes($_SESSION['name']) : '' }}";

    // Ensure jQuery is loaded
    if (typeof $ === 'undefined') {
        console.error("jQuery is not loaded. Please ensure jQuery is included.");
        return;
    }

    // Handle login form submission
    $(document).on("submit", "#login-form", function (e) {
        e.preventDefault(); // Prevent default form submission
        e.stopImmediatePropagation(); // Stop any other handlers

        const name = $("#name").val().trim();
        if (name === "") {
            // Translation: 'Vui lòng nhập tên' -> '이름을 입력해주세요'
            $("#login-error").text("이름을 입력해주세요");
            return false;
        }

        // Update UI immediately (with Korean text)
        currentUser = name;
        $("#chat-wrapper").html(`
            <div id="menu">
                <p class="welcome">안녕하세요, <b>${name}</b></p>
                <p class="logout"><a id="exit" href="javascript:void(0);">채팅 종료</a></p>
            </div>
            <div id="chatbox"></div>
            <form name="message" id="message-form">
                <input name="usermsg" type="text" id="usermsg" required="required" placeholder="메시지를 입력하세요..." />
                <input name="submitmsg" type="submit" id="submitmsg" value="전송" />
            </form>
        `);

        // Submit login to server via AJAX
        $.ajax({
            url: "{{ url()->current() }}",
            type: "POST",
            data: {
                _token: "{{ csrf_token() }}",
                name: name,
                enter: "입장" // Korean text sent to server (if needed)
            },
            success: function (response) {
                console.log("Login successful:", response);
                loadLog(); // Load chat log after successful login
            },
            error: function (xhr, status, error) {
                console.error("Login error:", xhr, status, error);
                // Translation: 'Lỗi khi đăng nhập. Vui lòng thử lại.' -> '로그인 오류입니다. 다시 시도하세요.'
                $("#login-error").text("로그인 오류입니다. 다시 시도하세요.");
                // Revert UI if login fails (with Korean text)
                $("#chat-wrapper").html(`
                    <div id="loginform">
                        <p>채팅을 위해 이름을 입력하세요!</p>
                        <form id="login-form" method="post">
                            <input type="hidden" name="_token" value="{{ csrf_token() }}">
                            <label for="name">이름 :</label>
                            <input type="text" name="name" id="name" value="${name}" />
                            <input type="submit" name="enter" id="enter" value="입장" />
                        </form>
                        <div id="login-error" class="error">로그인 오류입니다. 다시 시도하세요.</div>
                    </div>
                    <div id="chatbox"></div>
                `);
            }
        });

        return false;
    });

    // Handle message submission
    $("#chat-wrapper").on("submit", "#message-form", function (e) {
        e.preventDefault();
        const clientmsg = $("#usermsg").val().trim();
        if (clientmsg === "") {
            // Translation: 'Vui lòng nhập tin nhắn.' -> '메시지를 입력해주세요.'
            bootbox.alert({
                message: "메시지를 입력해주세요.",
                size: 'small',
                centerVertical: true,
                locale: 'ko', // Set locale to Korean for Bootbox
                closeButton: false,
                buttons: {
                    ok: {
                        className: 'btn-danger pulse-red'
                    }
                }
            });
            return false;
        }

        // Submit message to server using new database API
        $.post("{{ url('/api') }}/sendChatMessage", {
            roomCode: "{{ $roomCode }}",
            text: clientmsg,
            username: currentUser,
            _token: "{{ csrf_token() }}"
        }).done(function(response) {
            if (response.success) {
                loadLog(); // Reload messages immediately after sending
            }
        }).fail(function(xhr, status, error) {
            console.error("Error sending message:", xhr, status, error);
        });
        $("#usermsg").val("");
        return false;
    });

    // Handle logout
    $("#chat-wrapper").on("click", "#exit", function (e) {
        e.preventDefault();
        // Translation: 'Kết thúc phiên chat này?' -> '이 채팅 세션을 종료하시겠습니까?'
        bootbox.confirm({
            message: "이 채팅 세션을 종료하시겠습니까?",
            centerVertical: true,
            locale: 'ko', // Set locale to Korean for Bootbox
            closeButton: false,
            buttons: {
                confirm: {
                    // Translation: 'Thoát' -> '종료'
                    label: '<i class="fas fa-check"></i> 종료',
                    className: 'btn-danger pulse-red'
                },
                cancel: {
                    // Translation: 'Hủy' -> '취소'
                    label: '<i class="fas fa-times"></i> 취소',
                    className: 'btn-dark text-light'
                }
            },
            callback: function (result) {
                if (result) {
                    // Update UI immediately (with Korean text)
                    currentUser = "";
                    $("#chat-wrapper").html(`
                        <div id="loginform">
                            <p>채팅을 위해 이름을 입력하세요!</p>
                            <form id="login-form" method="post">
                                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                <label for="name">이름 :</label>
                                <input type="text" name="name" id="name" value="" />
                                <input type="submit" name="enter" id="enter" value="입장" />
                            </form>
                            <div id="login-error" class="error"></div>
                        </div>
                        <div id="chatbox"></div>
                    `);

                    // Submit logout to server via AJAX
                    $.ajax({
                        url: "{{ url()->current() }}?logout=true",
                        type: "POST",
                        data: {
                            _token: "{{ csrf_token() }}",
                            logout: true
                        },
                        success: function (response) {
                            console.log("Logout successful:", response);
                            loadLog(); // Load chat log after successful logout
                        },
                        error: function (xhr, status, error) {
                            console.error("Logout error:", xhr, status, error);
                            // Translation: 'Lỗi khi thoát. Vui lòng thử lại.' -> '종료 오류입니다. 다시 시도하세요.'
                            $("#login-error").text("종료 오류입니다. 다시 시도하세요.");
                        }
                    });
                }
            }
        });
    });

    // Load chat log from database
    function loadLog() {
        var oldscrollHeight = $("#chatbox")[0].scrollHeight - 20; // Scroll height before the request
        $.ajax({
            url: "{{ url('/api') }}/getChatMessages",
            type: "POST",
            data: {
                roomCode: "{{ $roomCode }}",
                _token: "{{ csrf_token() }}"
            },
            cache: false,
            success: function (data) {
                if (data.success) {
                    let html = '';
                    data.messages.forEach(function(msg) {
                        html += "<div class='msgln'>";
                        html += "<span class='chat-time'>" + msg.formatted_date + "</span> ";

                        if (msg.type === 'system') {
                            html += "<span class='welcome-info'>" + msg.message + "</span>";
                        } else if (msg.type === 'enter') {
                            // Translation: 'Người dùng' -> '사용자'
                            html += "<span class='enter-info'>사용자 <b class='user-name-enter'>" + msg.username + "</b> " + msg.message + "</span>";
                        } else if (msg.type === 'leave') {
                            // Translation: 'Người dùng' -> '사용자'
                            html += "<span class='left-info'>사용자 <b class='user-name-left'>" + msg.username + "</b> " + msg.message + "</span>";
                        } else {
                            html += "<b class='user-name'>" + msg.username + "</b>: " + msg.message;
                        }
                        html += "<br></div>";
                    });

                    $("#chatbox").html(html); // Insert chat log into the #chatbox div
                    var newscrollHeight = $("#chatbox")[0].scrollHeight - 20; // Scroll height after the request
                    if (newscrollHeight > oldscrollHeight) {
                        $("#chatbox").animate({ scrollTop: newscrollHeight }, 'normal'); // Autoscroll to bottom
                    }
                }
            },
            error: function (xhr, status, error) {
                console.error("Error loading chat log:", xhr, status, error);
            }
        });
    }

    // Poll for new messages every 1 second
    setInterval(loadLog, 1000);
});
</script>
