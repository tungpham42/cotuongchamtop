...
if (isset($_GET['logout'])) {
    if (isset($_SESSION['name'])) {
        $logout_message = "<div class='msgln'><span class='chat-time'>".date("Y-m-d | H:i:s")."</span> <span class='left-info'>Người dùng <b class='user-name-left'>". $_SESSION['name'] ."</b> đã rời khỏi phòng chat.</span><br></div>";
...
<div id="chat-wrapper">
    ...
    <div id="chatbox">
        @php
        if (file_exists($room_path) && filesize($room_path) > 0) {
            $contents = file_get_contents($room_path);
            echo $contents;
        }
        @endphp
    </div>
