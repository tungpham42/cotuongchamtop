                ...
                <div class="card-body">
                    <form action="{{ localized_url('tournaments.update', ['slug' => $tournament->slug]) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group text-dark">
                            ...
                            <input type="text" name="name" class="form-control bg-light text-dark border-dark" value="{{ $tournament->name }}" required>
                        </div>
                        ...
                        <div class="form-group text-dark">
                            ...
                            <textarea name="description" class="form-control bg-light text-dark border-dark" rows="3">{{ $tournament->description }}</textarea>
                        </div>

                        <div class="row">
                            <div class="col-md-4 form-group text-dark">
                                <label>{{ __('Ngày Bắt Đầu') }} <span class="text-danger">*</span></label>
                                <input type="datetime-local" name="start_date" class="form-control bg-light text-dark border-dark" value="{{ date('Y-m-d\TH:i', strtotime($tournament->start_date)) }}" required>
                            </div>

                            <div class="col-md-4 form-group text-dark">
                                <label>{{ __('Số lượng kỳ thủ') }} <span class="text-danger">*</span></label>
                                <input type="number" name="max_players" class="form-control bg-light text-dark border-dark" min="2" max="16" value="{{ $tournament->max_players }}" required>
                            </div>

                            <div class="col-md-4 form-group text-dark">
                                <label>{{ __('Trạng thái') }} <span class="text-danger">*</span></label>
                                <select name="status" class="form-control bg-light text-dark border-dark" required>
                        ...
                        <div class="mt-4 text-right">
                            <a href="{{ localized_url('tournaments.show', ['slug' => $tournament->slug]) }}" class="btn btn-outline-light text-dark mr-2">{{ __('Hủy') }}</a>
