...
$(document).ready(function () {
    ...
    let chatInterval = null; // Track the polling interval so we can stop it later
    ...
    // Function to re-render the login form dynamically
    ...
    if (currentUser !== "") {
        loadLog(true);
        chatInterval = setInterval(() => loadLog(false), 1500); // Store interval
    }
    ...
    $(document).on("submit", "#login-form", function (e) {
        ...
        $.ajax({
            ...
            success: function () {
                loadLog(true);
                // Clear any existing intervals before creating a new one
            ...
            callback: function (result) {
                if (result) {
                    $.ajax({
                        ...
                        success: function () {
                            // --- REFACTOR: Avoid page reload ---
                            if (chatInterval) {
                                clearInterval(chatInterval); // Stop AJAX polling
                                chatInterval = null;
                            }
                            currentUser = "";         // Clear current user memory
                            renderedMessageCount = 0; // Reset read messages count
                            renderLoginForm();        // Restore the login UI
                        }
                    });
                }
            }
