...
@section('aboveBoard')
    @php
        // Resolve the identities for non-logged-in vs logged-in match types
    ...
    @if (!empty($hostToRender))
        {{-- Display host & guest titles regardless of role when any room host identifier exists --}}
        <h5 id="room-title" class="text-center my-1">
            <span id="host-title">{!! $userPresenter->renderPlayerName($hostToRender, true) !!}</span>
            <span id="guest-title">{!! $userPresenter->renderPlayerName($guestToRender, true) !!}</span>
        </h5>
    @elseif ($role === 'watch')
        <h5 class="text-center my-1" data-toggle="tooltip" data-placement="top" title="{{ __('Quan sát hai kỳ thủ đang chơi') }}">
            {{ __('Bạn đang theo dõi') }}
        </h5>
    @elseif ($role === 'random')
        <h5 class="text-center my-1" data-toggle="tooltip" data-placement="top" title="{{ __('Bạn đang đi quân đen') }}">
            {{ __('Bạn đang đánh ngẫu nhiên') }}
        </h5>
    @else
        @php
        ...
        @endphp
        <h5 class="text-center my-1" data-toggle="tooltip" data-placement="top" title="{{ __($tooltip) }}">
            {{ __($titleMap[$role] ?? '') }}
        </h5>
    @endif
    ...
    @if ($role !== 'watch')
        ...
        @if(in_array($role, ['host', 'guest', 'red', 'black', 'random']))
    ...
    <div class="room-control-panel card shadow-lg mb-4 mx-auto" style="max-width: 650px; border-radius: 12px; background: var(--glass-bg-dark); backdrop-filter: var(--glass-blur);">
        <div class="card-body p-3 text-center">
            ...
            @if ($inviteRoute)
                ...
                <div id="copy-url-invite" class="input-group mb-3 w-75 mx-auto pulse-gold" data-toggle="tooltip" data-placement="bottom" data-original-title="{{ __('Ấn để sao chép') }}" style="box-shadow: 0 4px 15px rgba(0,0,0,0.6); border-radius: 6px;">
