...
<p class="w-100 text-center mt-2">
  <a class="w-25 mx-auto btn btn-success btn-sm" target="_blank" href="/phong/{{ $roomCode }}/duoc-moi"><i class="fad fa-external-link-alt"></i> {{ __("Mời bạn bè cùng chơi") }}</a>
</p>
...
<p class="w-100 text-center mt-4">
  <a class="w-25 btn btn-danger btn-lg" href="/phong/{{ $roomCode }}/do"><i class="fad fa-chess-clock-alt"></i> {{ __("Bên ĐỎ") }}</a>
  <a class="w-25 btn btn-dark btn-lg" href="/phong/{{ $roomCode }}/den"><i class="fad fa-chess-clock"></i> {{ __("Bên ĐEN") }}</a>
</p>
<script>
let board = null;
...
function manipulateRoom(roomCode) {
  $.ajax({
    ...
    if (gameTurn === 'r') {
      $('#game-status').removeClass('black').addClass('red').html('<i class="fal fa-chess-clock-alt"></i> {{ __("Tới lượt") }} ĐỎ');
    } else if (gameTurn === 'b') {
      $('#game-status').removeClass('red').addClass('black').html('<i class="fal fa-chess-clock"></i> {{ __("Tới lượt") }} ĐEN');
    }
  });
}
