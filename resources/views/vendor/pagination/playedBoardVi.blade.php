@if ($paginator->hasPages())
    <nav class="w-100 ml-3">
        <ul class="pagination">
            ...
            @else
                <li class="page-item">
                    <a class="page-link btn-md bg-dark text-light" href="{{ $paginator->previousPageUrl() }}&loai={{ __('van-da-dau') }}#{{ __('van-da-dau') }}" rel="prev"><i class="fas fa-chevron-left"></i> {{ __('Trước') }}</a>
                </li>
            @endif
                ...
                @if (is_array($element))
                    @foreach ($element as $page => $url)
                        ...
                        @elseif ($page === $paginator->currentPage() + 1 || $page === $paginator->currentPage() - 1 || $page === $paginator->lastPage() || $page === 1)
                            <li class="page-item"><a class="page-link btn-md bg-dark text-light" href="{{ $url }}&loai={{ __('van-da-dau') }}#{{ __('van-da-dau') }}">{{ $page }}</a></li>
                        @endif
            ...
            @if ($paginator->hasMorePages())
                <li class="page-item">
                    <a class="page-link btn-md bg-dark text-light" href="{{ $paginator->nextPageUrl() }}&loai={{ __('van-da-dau') }}#{{ __('van-da-dau') }}" rel="next">{{ __('Sau') }} <i class="fas fa-chevron-right"></i></a>
                </li>
            @else
                <li class="page-item disabled" aria-disabled="true">
